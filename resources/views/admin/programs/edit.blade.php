...
<style>
    ...
    .form-label.required::after { content: "*"; color: #22c55e; margin-left: 4px; } /* Warna bintang merah diubah jadi hijau */
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 3rem;">
            ...
            <div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nama_program" class="form-control @error('nama_program') border-green-500 @enderror" value="{{ old('nama_program', $program->nama_program) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="instansi" class="form-control" value="{{ old('instansi', $program->instansi) }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_mitra" class="form-control @error('id_mitra') border-green-500 @enderror" required>
                        @foreach($mitras as $mitra)
                            <option value="{{ $mitra->id }}" {{ old('id_mitra', $program->id_mitra) == $mitra->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control @error('tanggal_mulai') border-green-500 @enderror" value="{{ old('tanggal_mulai', \Carbon\Carbon::parse($program->tanggal_mulai)->format('Y-m-d')) }}" required>
                        @error('tanggal_mulai') <span class="text-xs text-green-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label class="form-label required">Tanggal Selesai</label>
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control @error('tanggal_selesai') border-green-500 @enderror" value="{{ old('tanggal_selesai', \Carbon\Carbon::parse($program->tanggal_selesai)->format('Y-m-d')) }}" required>
