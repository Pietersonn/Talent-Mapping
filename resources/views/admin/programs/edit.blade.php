        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 3rem;">
            ...
            <div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') border-red-500 @enderror" value="{{ old('nama', $Program->nama) }}" required>
                    @error('nama') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label required">Kode Program (Unik)</label>
                    <input type="text" name="kode_program" id="kode_program" class="form-control @error('kode_program') border-red-500 @enderror" value="{{ old('kode_program', $Program->kode_program) }}" required>
            ...
            <div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control @error('tanggal_mulai') border-red-500 @enderror" value="{{ old('tanggal_mulai', $Program->tanggal_mulai ? $Program->tanggal_mulai->format('Y-m-d') : '') }}" required>
                        @error('tanggal_mulai') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label class="form-label required">Tanggal Selesai</label>
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control @error('tanggal_selesai') border-red-500 @enderror" value="{{ old('tanggal_selesai', $Program->tanggal_selesai ? $Program->tanggal_selesai->format('Y-m-d') : '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="id_mitra" class="form-control" style="background-image: none;">
                        <option value="">-- Pilih Mitra (Opsional) --</option>
                        @foreach($mitras as $mitra)
                            <option value="{{ $mitra->id }}" {{ old('id_mitra', $Program->id_mitra) == $mitra->id ? 'selected' : '' }}>{{ $mitra->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="maks_peserta" class="form-control" value="{{ old('maks_peserta', $Program->maks_peserta) }}" placeholder="Kosongkan jika tidak terbatas" min="1">
                </div>
                ...
                <div class="form-group" style="margin-top: 2rem;">
                    ...
                    <div class="toggle-wrapper">
                        ...
                        <div>
                            <div style="font-weight: 600; font-size: 0.9rem; color: #0f172a;">Program Aktif</div>
