...
@section('title', $questionVersion->nama)

@push('styles')
<style>
    :root { --theme-color: #10b981; --theme-bg: #ecfdf5; --theme-hover: #059669; }
...
<div class="page-header">
    <div>
        ...
        <div class="text-sm text-slate-500 mt-1 ml-1">
            Bank Soal &rsaquo; {{ strtoupper($questionVersion->jenis) }} &rsaquo; <span class="font-semibold text-slate-700">{{ $questionVersion->versi }}</span>
        </div>
    </div>
    ...
    <div>
        <div class="detail-card">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <h2 class="text-xl font-bold text-slate-800 mb-1">{{ $questionVersion->nama }}</h2>
                ...
                <div class="flex gap-2">
                    @if(Auth::user()->role === 'admin')
                        @if(!$questionVersion->aktif && $statistics['total_questions'] >= ($questionVersion->jenis === 'st30' ? 30 : 50))
            ...
            <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4 flex-grow">
                ...
                <div class="text-slate-700 text-sm leading-relaxed">
                    {{ $questionVersion->deskripsi ?: 'Tidak ada deskripsi tambahan.' }}
                </div>
            </div>

            <div class="mt-auto pt-2">
                @php
                    $target = $questionVersion->jenis === 'st30' ? 30 : 50;
    ...
    <div>
        <div class="detail-card">
            ...
            <div class="stat-grid mb-4">
                ...
                <div class="stat-box">
                    <div class="stat-num">{{ $questionVersion->jenis === 'st30' ? '30' : '50' }}</div>
        ...
        <div class="detail-card" style="padding: 0; overflow: hidden;">
            <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <div class="flex gap-2">
                    @if(Auth::user()->role === 'admin')
                        @php
                            $indexRoute = $questionVersion->jenis === 'st30'
                                ? route('admin.questions.st30.index', ['version' => $questionVersion->id])
                                : route('admin.questions.tk.index', ['version' => $questionVersion->id]);
                        @endphp
                    @endif
                ...
                <table class="custom-table">
                    <thead>
                        <tr>
                            ...
                            <th width="150">{{ $questionVersion->jenis === 'st30' ? 'Tipologi' : 'Kompetensi' }}</th>
                            <th width="100">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            $questions = $questionVersion->jenis === 'st30'
                                ? $questionVersion->st30Questions->take(10)
                                : $questionVersion->talentCompetencyQuestions->take(10);
                        @endphp

                        @forelse($questions as $q)
                        <tr>
                            <td class="text-center font-mono font-bold text-slate-500">{{ $q->nomor }}</td>
                            <td>
                                @if($questionVersion->jenis === 'st30')
                                    {{ Str::limit($q->pernyataan, 80) }}
                                @else
                                    {{ Str::limit($q->teks_pertanyaan, 80) }}
                                    <div class="text-xs text-slate-400 mt-1">
                                        {{ $q->options ? $q->options->count() : 0 }} Opsi Jawaban
                                    </div>
                                @endif
                            </td>
                            <td>
                                @if($questionVersion->jenis === 'st30')
                                    <span class="tag tag-typology">{{ $q->kode_tipologi }}</span>
                                @else
                                    <span class="tag tag-competency">{{ Str::limit($q->kode_kompetensi, 15) }}</span>
                                @endif
                            </td>
                            <td>
                                @if($q->aktif)
