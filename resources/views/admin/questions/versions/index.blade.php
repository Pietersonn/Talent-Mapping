...
<div class="dashboard-grid">
            ...
            @if($activeVersions['st30'])
                <div class="bg-white/60 backdrop-blur-sm p-3 rounded-xl border border-violet-100">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="text-sm font-bold text-slate-700">{{ $activeVersions['st30']->nama }}</span>
                    </div>
                    <div class="text-xs text-slate-500 pl-1">
                        Versi {{ $activeVersions['st30']->versi }} &bull; {{ $activeVersions['st30']->st30_questions_count }}/30 Soal
                    </div>
                </div>
            @else
            ...
            <div class="table-scroll">
                <table class="custom-table" id="st30Table">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $version->aktif ? 'row-active' : '' }}">
                            <td><span class="font-mono text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">v{{ $version->versi }}</span></td>
                            <td>
                                <div class="font-semibold">{{ $version->nama }}</div>
                                <div class="text-xs text-slate-400">{{ $version->created_at->format('d/m/Y') }}</div>
                                <span style="display:none;">{{ $version->aktif ? 'aktif' : 'tidak aktif' }} ST30</span>
                            </td>
                            ...
                            <td class="text-right">
                                <div class="action-buttons">
                                    @if(Auth::user()->role === 'admin' && !$version->aktif)
                                        <button class="btn-activate" onclick="checkAndActivate('{{ $version->id }}', '{{ $version->nama }}', 'ST-30', {{ $version->st30_questions_count }}, 30)">
                                            Gunakan
                                        </button>
                                    @endif
                                    ...
                                    @if(Auth::user()->role === 'admin')
                                        ...
                                        @if(!$version->aktif)
                                            <button class="btn-icon btn-delete" title="Hapus" onclick="deleteVersion('{{ $version->id }}', '{{ $version->nama }}')">
    ...
    {{-- CARD TK (PENGGANTI SJT) --}}
    <div class="content-card">
        <div class="card-hero hero-sjt">
            <div class="hero-top">
                <div>
                    <div class="version-subtitle">TALENT KOMPETENSI</div>
                    <div class="version-title">Soal Talent Kompetensi (TK)</div>
                </div>
                <div class="hero-icon icon-sjt"><i class="fas fa-clipboard-check"></i></div>
            </div>

            @if($activeVersions['tk'])
                <div class="bg-white/60 backdrop-blur-sm p-3 rounded-xl border border-sky-100">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="badge-active"><i class="fas fa-check-circle"></i> SEDANG AKTIF</span>
                        <span class="text-sm font-bold text-slate-700">{{ $activeVersions['tk']->nama }}</span>
                    </div>
                    <div class="text-xs text-slate-500 pl-1">
                        Versi {{ $activeVersions['tk']->versi }} &bull; {{ $activeVersions['tk']->talent_competency_questions_count }}/50 Soal
                    </div>
                </div>
            @else
                <div class="text-sm text-slate-400 italic">Belum ada versi aktif.</div>
        ...
        <div class="list-wrapper">
            <div class="list-header">
                <span class="text-xs font-bold text-slate-400 uppercase">Daftar Versi</span>
                <a href="{{ route('admin.questions.tk.index') }}" class="text-xs font-bold text-sky-600 hover:text-sky-800">
                    KELOLA SOAL <i class="fas fa-arrow-right ml-1"></i>
                </a>
            </div>
            <div class="table-scroll">
                <table class="custom-table" id="tkTable">
                    <thead>
                        <tr>
                            <th>Ver</th>
                            <th>Nama Versi</th>
                    ...
                    <tbody>
                        @forelse($tkVersions as $version)
                        <tr class="{{ $version->aktif ? 'row-active' : '' }}">
                            <td><span class="font-mono text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">v{{ $version->versi }}</span></td>
                            <td>
                                <div class="font-semibold">{{ $version->nama }}</div>
                                <div class="text-xs text-slate-400">{{ $version->created_at->format('d/m/Y') }}</div>
                                <span style="display:none;">{{ $version->aktif ? 'aktif' : 'tidak aktif' }} TK</span>
                            </td>
                            <td class="text-center">
                                @if($version->talent_competency_questions_count == 50)
                                    <span class="text-green-600 font-bold text-xs"><i class="fas fa-check"></i> 50</span>
                                @else
                                    <span class="text-orange-500 font-bold text-xs">{{ $version->talent_competency_questions_count }}/50</span>
                                @endif
                            </td>
                            <td class="text-right">
                                <div class="action-buttons">
                                    @if(Auth::user()->role === 'admin' && !$version->aktif)
                                        <button class="btn-activate" onclick="checkAndActivate('{{ $version->id }}', '{{ $version->nama }}', 'TK', {{ $version->talent_competency_questions_count }}, 50)">
                                            Gunakan
                                        </button>
                                    @endif
...
<script>
    ...
    searchInput.on('keyup', function() {
        ...
        searchTimeout = setTimeout(() => {
            ...
            $('#tkTable tbody tr').filter(function() {
                $(this).toggle($(this).text().toLowerCase().indexOf(query) > -1)
            });
