...
.part-compact .form-select { height: 34px; padding-top: .125rem; padding-bottom: .125rem; }
.part-compact .input-group-text { height: 34px; }
.part-compact .btn-sm { padding: .25rem .6rem; }
.part-compact .card { border-radius: 12px; }
.part-compact .table thead th { white-space: nowrap; }
.part-compact .badge.rounded-pill { min-width: 42px; font-weight: 700; }
@media (max-width: 991.98px){
  .part-compact .filters-actions { justify-content: flex-start !important; margin-top: .25rem; }
}
</style>
      ...
      <form method="GET" id="filterForm">
        <div class="row g-2 align-items-end">
          ...
          <div class="col-12 col-lg-2">
            ...
            <div class="input-group input-group-sm">
              <input type="text" id="q" name="q" class="form-control" placeholder="Name / Email" value="{{ $q }}">
          ...
          <div class="col-12 d-flex justify-content-lg-end filters-actions">
            <div class="d-flex gap-2">
              <a href="{{ route('pic.participants.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
        ...
        <div class="text-muted small mt-1">
          <span class="me-3">Mode: <strong class="text-primary text-uppercase">{{ $mode }}</strong></span>
          @if($mode!=='all') <span class="me-3">Count: <strong>{{ $n }}</strong></span> @endif
          @if($eventId)   <span class="badge bg-info-subtle text-dark border">Event filtered</span> @endif
          @if($instansi)  <span class="badge bg-secondary-subtle text-dark border">Org: “{{ $instansi }}”</span> @endif
          @if($q)         <span class="badge bg-secondary-subtle text-dark border">Search: “{{ $q }}”</span> @endif
        </div>
      </form>
    </div>
  </div>
        ...
        <tbody>
          ...
          @forelse(($rows ?? []) as $i => $r)
            ...
            <tr>
              ...
              <td>
                @if(!empty($r->pdf_path))
                  <a href="{{ route('pic.participants.result-pdf', $param) }}" class="btn btn-sm btn-outline-primary">
...
(function(){
  ...
  function updateCountState(){
    if (modeSel.value === 'all') { countInput.disabled = true; }
