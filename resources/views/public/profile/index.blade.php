...
@section('content')
    <div class="tmprof-wrapper">
        <div class="tmprof-header">
            <h1 class="tmprof-title">Hai, {{ auth()->user()->nama ?? auth()->user()->name }}</h1>
        ...
        @if (session('warning'))
            <div class="tmprof-alert tmprof-alert--warning">{{ session('warning') }}</div>
        @endif

        @if (session('error'))
            <div class="tmprof-alert tmprof-alert--error">{{ session('error') }}</div>
        @endif
                ...
                <div class="tmprof-resend">
                    <form class="tmprof-form" method="POST" action="{{ route('profile.resend.request') }}">
                        ...
                        <select name="test_result_id" class="tmprof-select" required>
                            ...
                            @foreach ($results as $r)
                                <option value="{{ $r->id }}">
                                    {{-- PERBAIKAN UTAMA: Gunakan ->nama, bukan ->nama_program --}}
                                    {{ optional(optional($r->testSession)->program)->nama ?? 'Hasil Mandiri' }}

                                    {{-- Mengambil tanggal format (d M Y) --}}
                                    ({{ optional($r->laporan_dibuat_pada ?? $r->created_at)->format('d M Y') ?? '—' }})
                                </option>
                            @endforeach
                        </select>

                        <label class="tmprof-label">Catatan (opsional)</label>
                        <textarea name="note" class="tmprof-textarea" rows="3"
                            placeholder="Contoh: minta dikirim ulang ke email akun saya / butuh salinan terbaru">{{ old('note') }}</textarea>
                        ...
                        @forelse ($resendRequests as $req)
                            <div class="tmprof-request">
                                <div class="tmprof-request__meta">
                                    <span class="tmprof-chip tmprof-chip--{{ $req->status }}">{{ ucfirst($req->status) }}</span>
                                    <span class="tmprof-request__date">{{ optional($req->updated_at)->format('d M Y H:i') }}</span>
                                </div>
                                <div class="tmprof-request__email">
                                    Tujuan: {{ optional($req->user)->email ?? auth()->user()->email }}
                                </div>
                                @if ($req->catatan)
                                    <div class="tmprof-request__reason">Catatan: “{{ $req->catatan }}”</div>
                                @endif
                                @if ($req->approved_at)
                                    <div class="tmprof-request__reason">Disetujui: {{ \Illuminate\Support\Carbon::parse($req->approved_at)->format('d M Y H:i') }}</div>
                                @endif
